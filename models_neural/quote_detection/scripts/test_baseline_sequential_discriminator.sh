...
katie compute run \
        ...
        $worker_args \
        --python-module models_neural.quote_detection.eval \
        ...
        --do_train \
        --do_eval \
        --train_data_file_s3 data/sentence-df-for-discourse-modeling.csv.gz \
        --notes "Score sentence edits" \
        --freeze_transformer \
        --discriminator_path './trial-Roberta, high level labels__epoch=08-f1_macro=0.65.ckpt' \
        --processed_data_fname 'experiments/edit_scores.txt' \
        --context_layer 'lstm' \
