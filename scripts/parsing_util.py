...
def query_yago_for_organization(org):
    """
        Query YAGO for an organization name and return rdfs:type connections.
    """
    ...
    if 'type' in results_df:
        results_df['val'] = (results_df['type']
             .apply(lambda x: x['value'])
                             .str.replace('http://yago-knowledge.org/resource/', '')
...
def get_contiguous_organizations(sent_to_ents=None, words=None, ents=None):
    """
        Return continuous ORGANIZATIONS mentioned in a sentence.
    
    """
    if not sent_to_ents and (not words and not ents):
        raise('Provide <sent_to_ents> or <words> and <ents>')

    if not sent_to_ents:
        assert(len(words) == len(ents))
        sent_to_ents = zip(words,)
    
    in_organization = False
    organizations = []
    for word, ent in sent_to_ents:
        if in_organization:
            if (ent == 'ORGANIZATION'):
