...
# https://stackoverflow.com/a/38162096/2056246
# Get start, stop index pairs for islands/seq. of 1s
def get_start_stop_from_sequence(arr):
    """
    Get the start and stop positions of the longest subsequence of ones in an array.
    ...
    idx_pairs = np.where(np.diff(np.hstack(([False], arr == 1, [False]))))[0].reshape(-1, 2)
    island_lengths = np.diff(idx_pairs, axis=1)
    longest_island_id = island_lengths.argmax()
    start, end = idx_pairs[longest_island_id]
    return start, end - 1


def compute_metrics_one_substring(eval_preds):
    f1s, es = [], []
    batch_logits, batch_labels = eval_preds
    ...
    for logits, labels in zip(batch_logits, batch_labels):
        ...
        s_pred, e_pred = get_start_stop_from_sequence(preds)
        s_label, e_label = get_start_stop_from_sequence(labels)
...
def compute_metrics(eval_preds):
    ...
    for logits, labels in zip(batch_logits, batch_labels):
        ...
        preds = (logits > 0).astype(int)
        labels = labels.astype(int)
        f1_all.append(f1_metric.compute(predictions=preds, references=labels)['f1'])
        try:
            s_pred, e_pred = get_start_stop_from_sequence(preds)
            s_label, e_label = get_start_stop_from_sequence(labels)
            f1_sequence.append( sequence_f1(s_pred, e_pred, s_label, e_label) )
            es.append( sequence_overlap(s_pred, e_pred, s_label, e_label))
        except ValueError:
            f1_sequence.append(0)
            es.append(0)

    return {
        'f1_all': np.mean(f1_all),
        'f1_sequence': np.mean(f1_sequence),
        'e': np.mean(es)
    }
...
def freeze_hf_model(model, freeze_layers, model_type='bert'):
...
if __name__ == '__main__':
    ...
    from transformers import AutoModelForTokenClassification as ModelClass
    from token_classification_dataset import TokenClassificationDataset as DatasetClass, collate_fn
    ...
    model = ModelClass.from_pretrained(model_args.model_name_or_path, cache_dir=model_args.cache_dir)# , config=config)
    freeze_hf_model(model, model_args.freeze_layers)
